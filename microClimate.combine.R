...
for(i in 1:(length(names2[,1]))){
  ...
  dF.3$patchSize<-ns[ns$name==as.character(names2[i,"Plot"]),"PatchSize"]
...
for(i in 1:(length(dnames[,1]))){
  ...
  dF.3$patchSize<-ns[ns$name==as.character(dnames[i,"Plot"]),"PatchSize"]
...
colnames(vpd)<-c("month","max","name","plotcode","dist")
#remove -Inf
...
colnames(dF)<-c("month","ah","name","elevation","psize","kebele","eclass","tavg","tmax","tmin","vpd","Cavg","Nov","Jul")
