...
#save Tavg values
write.csv(et.1,paste0(getwd(),"/summary/Daily_Tavg.csv"))
...
et.0$PatchSize<-as.character(ns[match(et.0$Plot,ns$name),"PatchSize"])
ns$elev.class<-"Low"
...
#save Tdiff values
write.csv(et.0,paste0(getwd(),"/summary/Daily_Tdiff_byplot.csv"))
...
irates<-data.frame(cbind(as.character(ns[grep("Met_",ns$name,invert=T),"name"]),ns[grep("Met_",ns$name,invert=T),"elevation"]),stringsAsFactors = F)
colnames(irates)<-c("plot","elevation")
irates$elev.class<-"Low"
irates[as.numeric(irates$elevation)>1600,"elev.class"]<-"High"
...
#identify hours
met.low$hour<-cut(met.low$date,breaks="hour",labels=F)
met.high$hour<-cut(met.high$date,breaks="hour",labels=F)
